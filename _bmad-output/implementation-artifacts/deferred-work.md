...
## Deferred from: code review of story 9.1 (2026-06-12)

- **Dependency version mismatch: `pydantic` pinned `==2.5.0` in `backend/requirements.txt` vs `>=1.10,<2.0` in `backend/pyproject.toml`** (the live venv runs 1.10.26). Pre-existing, not caused by 9.1 — but 9.1's validators (`@validator`, `@root_validator(skip_on_failure=True)`, `Field(gt=, le=, ge=)`) and the numeric-coercion behavior depend on which version actually installs. A v2 install would change behavior (and break the `root_validator` import path). Reconcile the two pins to a single source of truth; verify the suite under whichever version is canonical. **Importance: elevated** — affects correctness of this module.
- **No startup cross-check that registry certification ids match the loaded exercises' `exam` values.** The model documents the invariant "id == existing exam value" but never verifies it against the corpus loaded in the same `lifespan`. A typo'd id surfaces only later as a request-time `CertificationConfigError`. Intentionally out of scope for 9.1 (additive) — belongs to **Story 9.4** (certification-scoped validation & content mapping).
- **Out-of-scope inclusion: the `main.py` `get_session` docstring hunk about a `"seen"` field belongs to Story 7.6**, committed together with 9.1 in `2e4e9eb`. Documentation-only, no behavior change; no action for 9.1.
- **AC5 "every existing test stays green" not observable in a clean full-suite run** because uncommitted/entangled Story 7.6 WIP leaves 2 tests failing (`test_code_completion_seen_is_always_false`, `test_post_entry_shape_matches_get`). Neither touches certifications; re-confirm a green full suite once 7.6 lands.
